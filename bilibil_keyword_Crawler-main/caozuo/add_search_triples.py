...
@app.route('/entities', methods=['GET', 'POST'], endpoint='entities_page')
def list_entities_route():
    ...
    if request.method == 'POST' and 'add_entity_btn' in request.form:
        ...
        return redirect(url_for('entities_page', page=page))
    if query:
...
@app.route('/entities/edit/<int:nid>', methods=['GET', 'POST'], endpoint='edit_entity_page')
def edit_entity(nid):
    page = int(request.args.get('page', 1))
    query = request.args.get('q', '').strip()
    if request.method == 'POST':
        ...
        if new_name:
            ...
            flash('已更新实体', 'success')
            return redirect(url_for('entities_page', page=page, q=query))
        else:
...
@app.route('/entities/delete/<int:nid>', methods=['POST'], endpoint='delete_entity_page')
...
@app.route('/relations', methods=['GET', 'POST'], endpoint='relations_page')
def list_relations_route():
    ...
    if request.method == 'POST' and 'add_relation_btn' in request.form:
        ...
        return redirect(url_for('relations_page', page=page))
    if query:
...
@app.route('/relations/edit/<rel>', methods=['GET', 'POST'], endpoint='edit_relation_page')
def edit_relation(rel):
    ...
    if request.method == 'POST':
        ...
        if new_name:
            ...
            flash('已更新关系', 'success')
            return redirect(url_for('relations_page', page=page, q=query))
        else:
            flash('请输入关系名称', 'warning')
    return render_template('edit_relation.html', rel=rel, page=page, query=query)


@app.route('/relations/delete/<rel>', methods=['POST'], endpoint='delete_relation_page')
